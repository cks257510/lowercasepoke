(function () {
  ...
  function pickSecondRewardBase(config) {
    const legendaryPool = getLegendaryRewardPool();
  ...
  function createRewardRuntime(basePokemon, level) {
    const runtime = core().createRuntimePokemon(basePokemon, level);
    let safety = 8;
    while (safety > 0 && runtime?.base?.evolution?.nextEvoId) {
      const evoLevel = Number(runtime.base.evolution.evoLevel || 0);
      if (!evoLevel || Number(runtime.level || 0) < evoLevel) break;
      const nextBase = core().state.pokemonById.get(runtime.base.evolution.nextEvoId);
      if (!nextBase) break;
      core().syncRuntimePokemon?.(runtime, nextBase);
      safety -= 1;
    }
    runtime.currentHp = runtime.maxHp;
    return runtime;
  ...
  function grantDungeonRewards(playerId, config) {
    ...
    const secondBase = pickSecondRewardBase(config);
    ...
    return `
      <section class="panel-card">
        <div class="section-title-row">
          <div>
            <h1 class="section-title" style="font-size:1.22em;">🌋 던전</h1>
            <p class="section-caption" style="font-size:1.08em;">던전은 리그 배틀과 같은 화면과 배틀 시스템을 사용합니다. 각 던전에서는 랜덤 적 1마리와 보스 1마리가 차례로 등장합니다.</p>
          </div>
          <button type="button" class="chip-btn" data-dungeon-start="1">던전 입장</button>
        </div>
        <div class="summary-grid">
          <div class="summary-card"><div class="summary-label">현재 누적 리그 배틀</div><div class="summary-value">${currentBattleCount(activePlayerId)}회</div></div>
          <div class="summary-card"><div class="summary-label">현재 선택</div><div class="summary-value">${config.name} · ${availability.open ? '도전 가능' : '잠김'}</div></div>
        </div>
        <div class="subsection"><div class="subheading"><h3>던전 목록</h3><span>각 던전의 조건과 보스를 확인하세요.</span></div><div class="placeholder-stack">${Object.values(DUNGEONS).map((entry) => renderCard(entry, activePlayerId)).join('')}</div></div>
        <div class="subsection"><div class="subheading"><h3>도전 조건 / 초기화</h3><span>시즌 초기화와 보상 정보</span></div><div class="placeholder-stack">
                    <div class="placeholder-card"><p>챔피언 리그가 끝나고 새 시즌이 시작되면 던전 해금에 쓰이는 누적 리그 배틀 횟수는 초기화됩니다.</p></div>
  ...
  function bindCategory(root) {
    ...
    root.querySelectorAll('[data-dungeon-start]').forEach((button) => {
      button.addEventListener('click', () => startSelectedDungeon());
    });
