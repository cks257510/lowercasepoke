(function () {
  ...
  function championTeams(trainerId) {
    const mapping = {
      ash: ['라이츄', '이상해꽃', '거북왕'],
      taehong: ['크로뱃', '갸라도스', '포푸니라'],
      cynthia: ['화강돌', '밀로틱', '한카리아스'],
      lance: ['리자몽', '파비코리', '망나뇽']
    };
    return mapping[trainerId] || [];
  }

  function buildNpcNames(trainerId) {
    const core = POKEBATTLE.core;
    if (['ash', 'taehong', 'cynthia', 'lance'].includes(trainerId)) {
      return championTeams(trainerId).filter((name) => safeFindByName(name));
    }
    const pool = shuffle(core.getTrueStarterCandidates().filter((pokemon) => core.isUnevolvedCandidate(pokemon)));
    return pool.slice(0, 3).map((p) => p.nameKo);
  }

  function ensureNpcSquad(trainerId) {
    const core = POKEBATTLE.core;
    if (trainerId === 'p1' || trainerId === 'p2') return core.getPlayer(trainerId);
    if (state.npcCache[trainerId]) return state.npcCache[trainerId];
    const npc = { id: trainerId, name: TRAINERS[trainerId]?.name || trainerId, squad: [], reserve: [], bag: { consumables: [] } };
    const names = buildNpcNames(trainerId);
    npc.squad = names.map((name) => safeFindByName(name)).filter(Boolean).map((base) => core.createRuntimePokemon(base, 5));
    const holdables = core.getHoldableCatalog ? core.getHoldableCatalog() : [];
    npc.bag = npc.bag || { consumables: [] };
    npc.bag.consumables = [{ id: 'good_potion', nameKo: '좋은상처약', amount: 1, category: '소비아이템', description: 'HP를 60 회복한다.' }];
  ...
  function getPlayerOrNpc(id) {
    return id === 'p1' || id === 'p2' ? POKEBATTLE.core.getPlayer(id) : ensureNpcSquad(id);
  }
  ...
  function prepareBattleTeams(playerId, opponentId, stage) {
    const core = POKEBATTLE.core;
    const player = getPlayerOrNpc(playerId);
    const opponent = getPlayerOrNpc(opponentId);
    ...
    if (opponentId !== 'p1' && opponentId !== 'p2') core.setPlayerTeamLevel(opponent, targetEnemyLevel);
    if (playerId !== 'p1' && playerId !== 'p2') core.setPlayerTeamLevel(player, targetEnemyLevel);
    core.healPlayerTeam(player);
    core.healPlayerTeam(opponent);
  }
  ...
  function renderBattleRow(battle) {
    return `<div style="padding:10px 12px;border-radius:14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);">
      <div style="font-weight:800;margin-bottom:4px;">경기${battle.battleNo || 1}</div>
      <div>${battleLabel(battle)}</div>
      <div style="margin-top:4px;color:#9fb0c7;">${battle.done ? `완료 · 승자 ${TRAINERS[battle.winnerId]?.name || battle.winnerId}` : '대기 중'}</div>
    </div>`;
  }
    ...
    return `
      <section class="panel-card">
        <div class="section-title-row">
          <div>
            <h1 class="section-title" style="font-size:1.22em;">🏆 리그</h1>
            <p class="section-caption" style="font-size:1.08em;">MATCH를 시작하려면 던전을 1회씩 클리어해야합니다. 한 시즌은 슈퍼리그를 마친 뒤 챔피언 토너먼트로 이어집니다. 슈퍼리그 1위와 2위는 지우, 태홍, 난천, 목호와 함께 챔피언 리그 토너먼트에 참가합니다.</p>
          </div>
          <button type="button" class="chip-btn" data-league-start="1">MATCH 시작</button>
        </div>
        <div class="summary-grid">
          <div class="summary-card"><div class="summary-label">현재 시즌</div><div class="summary-value">시즌 ${state.season} · ${state.stage === 'super' ? '슈퍼리그' : '챔피언 리그'}</div></div>
          <div class="summary-card"><div class="summary-label">다음 경기</div><div class="summary-value">${nextBattle ? battleLabel(nextBattle) : '진행 가능한 경기가 없음'}</div></div>
        </div>
        <div class="subsection"><div class="subheading"><h3>리그 일정</h3><span>좌우 버튼으로 MATCH 이동</span></div><div class="placeholder-stack">${renderScheduleSection()}</div></div>
        <div class="subsection"><div class="subheading"><h3>트레이너 순위</h3><span>승, 패, 점수</span></div><div class="placeholder-stack">${renderTrainerStandings()}</div></div>
        <div class="subsection"><div class="subheading"><h3>포켓몬 순위</h3><span>Top 3만 표시</span></div><div class="placeholder-stack">${renderPokemonRankings()}</div></div>
        ${helper}
      </section>`;
  }
  ...
  function syncNpcLevels() {
    ...
    Object.keys(state.npcCache).forEach((trainerId) => {
      ...
      const reference = Math.max(getReferencePlayerLevel('p1'), getReferencePlayerLevel('p2'));
